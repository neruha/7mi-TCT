...
public class TctItemWolfSword implements CustomSpecialItem {
    ...
    public void onAttackPlayer(Player attacker, Player target) {
        if(target != null && attacker != null) {
            Plugin plugin = NanamiTctApi.plugin;
            PlayerData data = NanamiTctApi.utilities.getPlayerData(target.getUniqueId());
            PlayerData data_ = NanamiTctApi.utilities.getPlayerData(attacker.getUniqueId());
            if(!data.isSpectator()) {
                ...
                if (data_.getRole() == GameRoles.FOX) {
                    ...
                    if (data_.getWatcher() != null) {
                        if (data_.getWatcher().getCountFox() <= NanamiTctApi.config.getConfig().getInt("fox-reveal-time", 60)) {
                            if (data_.getWatcher().getCountFox() > 0) {
                                data_.getWatcher().setCountFox(NanamiTctApi.config.getConfig().getInt("fox-reveal-time", 60));
                            }
                        }
                    }
                }
                if (attacker.getInventory().getItemInMainHand() != null) {
                    ...
                    if(angleInDegrees <= 60 && angleInDegrees >= -60) {
                        ...
                        if(MySQLStatus.isSqlEnabled() && NanamiTctApi.playerStats != null) {
                            PlayerStat stat = NanamiTctApi.playerStats.getStat(attacker.getUniqueId());
