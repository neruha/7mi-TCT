...
public class PlayerWatcher {
    ...
    public PlayerWatcher(TctGame game, Player player) {
        ...
        this.objective = this.scoreboard.registerNewObjective(NanamiTctApi.utilities.resetColor(player.getName()), "dummy");
    ...
    public void startWatchWith10Ticks() {
        runnable10 = new BukkitRunnable() {
            ...
            public void run() {
                ...
                setObjective(getScoreboard().registerNewObjective(NanamiTctApi.utilities.resetColor(getPlayer().getName()), "dummy"));
                getObjective().setDisplayName(Reference.TCT_SIDEBAR_TITLE);
                getObjective().setDisplaySlot(DisplaySlot.SIDEBAR);
                setPlayerData(getGame().getReference().PLAYERDATA.get(getPlayer().getUniqueId()));
            ...
            public void run() {
                if(!data.isSpectator()) {
                    for(Player p : Bukkit.getOnlinePlayers()) {
                        PlayerData data = game.getReference().PLAYERDATA.get(p.getUniqueId());
                        if(!data.isInvisible()) {
                            if (data.isSpectator()) {
                                player.hidePlayer(game.getPlugin(), p);
                            } else {
                                if (NanamiTctApi.utilities.canSee(player, p)) {
                                    NanamiTctApi.utilities.showPlayer(player, p);
                                } else {
                                    NanamiTctApi.utilities.hidePlayer(player, p);
                                }
                            }
                        } else {
                            NanamiTctApi.utilities.hidePlayer(player, p);
                    ...
                    if (!getPlayerData().isSpectator() && getPlayerData().getRole().getTeam() != GameTeams.WOLVES) {
                        ...
                        for (CustomBlockData data : dataList) {
                            if (data.getCustomBlock().getItemStack().getItemMeta().getDisplayName().equalsIgnoreCase(CustomItems.LANDMINE.getItemStack().getItemMeta().getDisplayName())) {
                                if(data.isEnabled()) {
                                    double maxFar = NanamiTctApi.config.getConfig().getInt("landmine-range", 3);
    ...
    public void startCountFox() {
        int defaultCount = NanamiTctApi.config.getConfig().getInt("fox-reveal-time-default", 70);
        if(defaultCount > 0) {
