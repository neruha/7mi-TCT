...
public class TctCustomBlockData implements CustomBlockData {
    ...
    public void runTimer(TctCooldownType type) {
        if(type == CooldownTypes.HEAL_STATION) {
            setCooldown(NanamiTctApi.config.getConfig().getInt("heal-station-cooldown", 20) + 1);
            ...
            timer.runTaskLater(this.game.getPlugin(), NanamiTctApi.config.getConfig().getInt("landmine-cooldown", 5) * 20L);
        }
    }
